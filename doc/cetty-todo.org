...
** channel
	1，future增加线程不安全类型（服务器端）
        2, 增加AsioSocketChannel对于网络带宽小的情况下,异步发送的写缓冲存在积压,在连接断开后,可以释放所有的写缓冲.
...
* TODO cetty-config
config descript中增加父class信息，以及自身的一些信息
单个对象，使用mutable设值时，如果初始化为NULL，则存在问题
对象内嵌对象的class名称，应该包含其父对象（或者就可以包含namespace）*
...
(OK)1，channel删除的时机选择，自己close channel后，onread接收到错误消息后关闭
(OK)2，重复利用accept channel，需要重新open
