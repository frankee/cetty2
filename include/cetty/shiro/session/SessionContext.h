...
public:
     ...
     * @since 1.0
     */
    void setHost(const std::string &host) { if(!host.empty()) put(HOST, host); }
    ...
    void setSessionId(const std::string &sessionId) {
        if(!sessionId.empty()) put(SESSION_ID, sessionId);
    }


    void putAll(std::map<std::string, std::string> &context);
