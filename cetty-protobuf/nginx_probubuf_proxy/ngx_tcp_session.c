...
ngx_tcp_init_connection(ngx_connection_t *c)
{
    ...
    item->buffer = ngx_create_temp_buf(c->pool, pscf->buffer_size);
    if(item->buffer == NULL){
    	ngx_tcp_close_connection(c);
    	return;
    }
    ...
    c->buffer = ngx_create_temp_buf(c->pool, pscf->buffer_size);
    if (c->buffer == NULL) {
    	ngx_tcp_close_connection(c);
    	return;
    }
...
ngx_tcp_init_session(ngx_connection_t *c)
{
    ...
    if ( ngx_array_init(&s->upstreams, s->pool, 4, sizeof(ngx_tcp_upstream_t))
    		== NGX_ERROR){
    	ngx_tcp_finalize_session(s);
    	return;
    }
