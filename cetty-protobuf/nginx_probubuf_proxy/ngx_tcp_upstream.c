...
ngx_tcp_upstream_create(ngx_tcp_session_t *s) 
{
    ...
    u = ngx_array_push(&(s->upstreams));
    if (u == NULL) {
        return NGX_ERROR;
    }
    ngx_memzero(u, sizeof(ngx_tcp_upstream_t));

    u->peer.log = s->connection->log;
    u->peer.log_error = NGX_ERROR_ERR;
    u->index = s->upstreams.nelts - 1;
...
ngx_tcp_upstream_init(ngx_tcp_session_t *s)
{
    ...
    if(!s->upstream_cleanup) {
		cln = ngx_tcp_cleanup_add(s, 0);
		if(cln == NULL) {
			ngx_log_error(NGX_LOG_ERR, s->connection->log, 0,
						  "add cleanup for session failed");
...
ngx_tcp_upstream_connect(ngx_tcp_session_t *s, ngx_tcp_upstream_t *u) 
{
    ...
    ngx_tcp_reading_item_t *item = ngx_pcalloc(c->pool,
        sizeof(ngx_tcp_reading_item_t));
    if(item == NULL){
    	ngx_tcp_finalize_session(s);
        return;
    }

    plcf = (ngx_tcp_proxy_loc_conf_t *)u->proxy_conf;

    item->buffer = ngx_create_temp_buf(c->pool, plcf->buffer_size);
    if(item->buffer == NULL){
    	ngx_tcp_finalize_session(s);
        return;
    }
    ...
    c->buffer = ngx_create_temp_buf(c->pool, plcf->buffer_size);
    if (c->buffer == NULL) {
    	ngx_tcp_finalize_session(s);
        return;
    }
...
ngx_tcp_upstream_handler(ngx_event_t *ev) 
{
    ...
    for(i = 0; i < s->upstreams.nelts; ++i) {
    	if(!us[i].unused && us[i].peer.connection == c) {
    		u = &us[i];
    		break;
    	}
    }
