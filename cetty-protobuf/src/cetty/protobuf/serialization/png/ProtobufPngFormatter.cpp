...
void ProtobufPngFormatter::format(boost::int64_t value, std::string* str) {
    BOOST_ASSERT(false && "png not support format int64");
}

void ProtobufPngFormatter::format(boost::int64_t value, const ChannelBufferPtr& buffer) {
    BOOST_ASSERT(false && "png not support format int64");
}
...
void ProtobufPngFormatter::format(const std::string& value, std::string* str) {
    if (isPng(value)) {
        *str = value;
    }
}

void ProtobufPngFormatter::format(const std::string& value, const ChannelBufferPtr& buffer) {
    if (isPng(value)) {
