...
DefaultAsioServerSocketChannelConfig::DefaultAsioServerSocketChannelConfig(
...
bool DefaultAsioServerSocketChannelConfig::setOption(const std::string& key, const boost::any& value) {
...
const boost::optional<bool>& DefaultAsioServerSocketChannelConfig::isReuseAddress() const {
    if (reuseAddress) {
        return reuseAddress;
    }
...
bool DefaultAsioServerSocketChannelConfig::isReuseAddress(tcp::acceptor& acceptor) const {
    try {
...
void DefaultAsioServerSocketChannelConfig::setReuseAddress(tcp::acceptor& acceptor) {
    if (reuseAddress) {
...
const boost::optional<int>& DefaultAsioServerSocketChannelConfig::getReceiveBufferSize() const {
...
int DefaultAsioServerSocketChannelConfig::getReceiveBufferSize(tcp::acceptor& acceptor) const {
    try {
...
void DefaultAsioServerSocketChannelConfig::setReceiveBufferSize(tcp::acceptor& acceptor) {
    if (receiveBufferSize) {
        boost::system::error_code error;
...
void DefaultAsioServerSocketChannelConfig::setPerformancePreferences(int connectionTime, int latency, int bandwidth) {
    //socket.setPerformancePreferences(connectionTime, latency, bandwidth);
}

const boost::optional<int>& DefaultAsioServerSocketChannelConfig::getBacklog() const {
    return this->backlog;
}

void DefaultAsioServerSocketChannelConfig::setBacklog(int backlog) {
    if (backlog < 0) {
...
bool DefaultAsioServerSocketChannelConfig::channelHasReaderBuffer() const {
    return false;
}
